...
def createBarCodes(barcode_value, c, seatNo, ID, gender, first_name, last_name, faculty):
    ...
    while index < len(ID):
        ...
        c.drawString(150*mm, 3*mm, seatNo[index] + " - " + seatNo[index_last_of_page])
        ...
        while index_x < max_column:
            ...
            while index_y < max_row and index < len(ID):
                ...
                c.drawString(pos_x+10*mm, pos_y-35*mm - 6*mm, u"คณะ".encode('utf-8') + faculty)
                c.drawString(pos_x+10*mm, pos_y-35*mm - 6*2*mm, ID[index])
                c.drawString(pos_x+55*mm, pos_y-35*mm - 6*2*mm, u"ลำดับที่ ".encode('utf-8') + seatNo[index])
...
def createBarCodes_reserve(barcode_value, c, seatNo, ID, gender, first_name, last_name, faculty):
    """
    Create barcode from List and embed in a PDF
    one page has many cards
    """
    c.setFont("THSarabun", 12)
    ...
    while index < len(ID):
        ...
        c.translate(10*mm, 287*mm)
        c.drawString(0, 3*mm, faculty)   # print Faculty at the top of page
        index_last_of_page = min(len(seatNo)-1, index+max_row*max_column-1)
        c.drawString(150*mm, 3*mm, seatNo[index] + " - " + seatNo[index_last_of_page])

        # draw border
        # c.line(0,0,0,-card_height*max_row)
        # c.line(card_width,0,card_width,-card_height*max_row)
        # c.line(card_width*max_column,0,card_width*max_column,-card_height*max_row)
        index_x = pos_x = 0
        while(index_x < max_column+1):
            c.line(pos_x, 0, pos_x, -max_row*card_height)
            pos_x += card_width
            index_x += 1
        index_y = pos_y = 0
        while(index_y < max_row+1):
            c.line(0, pos_y, max_column*card_width, pos_y)
            pos_y -= card_height
            index_y += 1
        ...
        while index_x < max_column:
            ...
            while index_y < max_row and index < len(ID):
                #generage barcode 128
                barcode128 = code128.Code128(ID[index], barWidth=0.5*mm, barHeight=14*mm)
                barcode128.drawOn(c, pos_x-3*mm, (pos_y-17*mm))

                # draw
                c.setFont("THSarabun", 10)
                c.drawString(pos_x+3*mm, pos_y-24*mm, gender[index] + " " + first_name[index] + " " + last_name[index])  # len my name is 46 (space is 1 length)
                # c.drawString(pos_x+10*mm, pos_y-24*mm - 6*mm, u"คณะ".encode('utf-8') + faculty)
                c.drawString(pos_x+3*mm, pos_y-24*mm - 4*mm, ID[index])
                c.drawString(pos_x+20*mm, pos_y-24*mm - 4*mm, u"ลำดับที่ ".encode('utf-8') + seatNo[index])
